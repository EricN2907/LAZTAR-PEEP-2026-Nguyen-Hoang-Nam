...
![git init tạo repo trống tại D:/LazTar/demo-git](/images/day1/image1.png)
...
![git remote trước và sau khi thêm origin](/images/day1/image2.png)
...
![git remote add origin và git remote -v](/images/day1/image3.png)
![git remote -v xem fetch/push URL](/images/day1/image4.png)
...
![git clone repo về máy cục bộ](/images/day1/image5.png)
...
- Chạy `git log --oneline -3` — local đang có 3 commit mới nhất, `HEAD` và `origin/main` đang đồng bộ với nhau tại commit `f5373ea (Fix name)`.

![git log --oneline -3: local đang đồng bộ với remote](/images/day1/image6.png)

- Vào GitHub tạo commit mới tên **"Add instructions for building production site"** (hash `2855f46`). Lúc này remote đã có commit mà local chưa hay biết.

![GitHub đã có commit mới "Add instructions for building production site"](/images/day1/image7.png)

- Sau khi chạy `git fetch origin` → Git tải thông tin về, cập nhật `origin/main` lên `2855f46`. Chạy `git log main..origin/main --oneline` → thấy commit mới trên remote. Chạy `git diff main origin/main` → thấy nội dung thay đổi trong README. Nhưng nhánh `main` local vẫn chưa có commit đó.

![git fetch origin: thấy diff nhưng local chưa merge](/images/day1/image8.png)

> **Kết luận:** `git fetch` chỉ tải thông tin, **không** tự động hợp nhất vào nhánh đang làm việc.

![git log --oneline -3 sau fetch: local vẫn ở f5373ea, chưa có 2855f46](/images/day1/image9.png)
...
![git pull origin main: Fast-forward, README.md được cập nhật](/images/day1/image10.png)
...
![git fetch + git merge origin/main: Fast-forward thủ công](/images/day1/image11.png)
...
![git status: thấy file modified và untracked](/images/day1/image12.png)
...
![git status sau pull: "nothing to commit, working tree clean"](/images/day1/image13.png)
![git status sạch sau khi commit xong](/images/day1/image14.png)
...
![git branch: liệt kê nhánh, dấu * là nhánh hiện tại](/images/day1/image15.png)
![git branch + git switch: thấy danh sách rồi switch về main](/images/day1/image16.png)
...
![git checkout: thấy "Your branch is up to date with 'origin/main'"](/images/day1/image18.png)
...
![git commit --amend: hash thay đổi từ 887d8aa sang 1d481b8](/images/day1/image20.png)
...
**Trạng thái trước rebase**: `feature-demo` tách ra từ `main` — hai nhánh chạy song song.

![git switch -c feature-demo + commit "Add feature demo": lịch sử bắt đầu tách](/images/day1/image23.png)
![Sơ đồ lịch sử: main và feature-demo đang phân kỳ](/images/day1/image24.png)

**Sau rebase**: Chạy `git switch feature-demo` rồi `git rebase main` → lịch sử `feature-demo` được đặt lại lên đầu `main`, tuyến tính và gọn hơn.
...
![git rebase main: feature-demo "Current branch feature-demo is up to date"](/images/day1/image25.png)
...
- Sau khi `echo "# WIP note" >> README.md`, chạy `git status` → thấy `modified: README.md`. Đây là trạng thái **trước khi stash**.

![git status: README.md bị modified trước khi stash](/images/day1/image26.png)

- Chạy `git stash` → Git lưu thay đổi vào stash (`Saved working directory... WIP on feature-demo`). Sau đó `git stash pop` → thay đổi được khôi phục lại, `README.md` quay về trạng thái `modified`.

![git stash + git stash pop: lưu rồi khôi phục thay đổi](/images/day1/image27.png)
...
- Tạo nhánh `rebase-demoo` và tạo 3 commit liên tiếp (`fix A`, `fix B`, `fix C`) vào `README.md`. Chạy `git log --oneline -4` → thấy 3 commit rời nhau.

![tạo 3 commit fix A, fix B, fix C trên nhánh rebase-demoo](/images/day1/image28.png)

- Kết quả `git log --oneline -4` trên nhánh `rebase-demooo` — `HEAD` là commit `squash fix A B C` (đã gộp), bên dưới là commit gốc `fix C`, `fix B`, `fix A`.

![git log sau rebase -i: HEAD là "squash fix A B C"](/images/day1/image29.png)

- Màn hình Vim editor của `git rebase -i` — thấy 3 dòng `pick` với 3 commit. Đổi dòng 2 và 3 từ `pick` → `s` (squash) để gộp vào commit đầu.

![git rebase -i editor: pick/squash 3 commit](/images/day1/image30.png)
...
- Chuyển về `main` rồi chạy `git cherry-pick a9c80b8` → commit `squash fix A B C` được copy sang `main` với hash mới `1d93759`. Chạy `git log --oneline -3` → thấy commit đó xuất hiện trên `main`.
...
![git cherry-pick: commit "squash fix A B C" xuất hiện trên nhánh main](/images/day1/image31.png)
...
Working tree sạch; tại thời điểm chụp, `dev_1` local đang hơn `origin/dev_1` 15 commit. Đây là số commit chưa push, không phải số phút trong chính sách giữ chỗ.

![Kiểm tra working tree và các nhánh trong repo BE](/images/day1/image32.png)

Push `dev_1` và hai nhánh tính năng thành công; thiết lập upstream cho các nhánh mới.

![Push nhánh đích và hai nhánh tính năng lên GitHub](/images/day1/image33.png)
...
![Mở chính sách giữ chỗ trên nhánh tính năng và dev_1](/images/day1/image34.png)

GitHub báo **Can't automatically merge**. Diff bên dưới thể hiện thay đổi của nhánh tính năng từ bản gốc **5 → 10 phút**; đây không phải phép so sánh trực tiếp hai giá trị cuối **10 và 15 phút**.

![Chọn base và compare, GitHub phát hiện không thể tự merge](/images/day1/image35.png)
...
![Nhập tiêu đề cho Pull Request cập nhật thời gian giữ chỗ](/images/day1/image36.png)

PR **#12** liệt kê `docs/booking-policy.md` cần giải quyết. Nút **Resolve conflicts** khả dụng vì đây là conflict nội dung đơn giản.

![PR số 12 báo conflict tại booking-policy.md](/images/day1/image37.png)
...
![Web editor hiển thị hai giá trị 10 phút và 15 phút](/images/day1/image38.png)
...
![Giữ giá trị 15 phút và đánh dấu file đã giải quyết](/images/day1/image39.png)
...
![PR hết conflict sau khi tạo commit giải quyết trên web](/images/day1/image40.png)
...
![PR số 12 đã merge thành công vào dev_1](/images/day1/image41.png)
...
![So sánh nhánh cập nhật chính sách với dev_1](/images/day1/image42.png)
...
![Tạo Pull Request cập nhật chính sách đặt sân](/images/day1/image43.png)

PR **#13** báo ba file conflict. Khi di chuột vào nút **Resolve conflicts** bị vô hiệu hóa, GitHub hiển thị thông báo không thể giải quyết bằng web editor.

![GitHub thông báo conflict quá phức tạp để xử lý trong web editor](/images/day1/image44.png)
...
![Working tree sạch trước khi chuyển sang nhánh cập nhật chính sách](/images/day1/image45.png)
...
![Terminal báo conflict nội dung và conflict sửa/xóa sau lệnh merge](/images/day1/image46.png)
...
![Conflict chính sách hủy: Current 2 giờ, Incoming 4 giờ](/images/day1/image47.png)

![Conflict thanh toán: Current 30%, Incoming 50%](/images/day1/image48.png)
...
Chính sách hủy đã là **4 giờ**; Source Control không còn nhóm **Merge Changes**, các thay đổi nội dung đã được stage.

![Nội dung sau khi giải quyết và các file trong Staged Changes](/images/day1/image49.png)
...
![Kiểm tra diff đã stage trước khi hoàn tất merge](/images/day1/image50.png)
...
Commit `bd6a9e1` được tạo và push thành công; working tree sạch. Lệnh commit trên được giữ đúng theo ảnh thực hành. Tuy tên commit có cụm `retire legacy flow`, nội dung commit thực tế vẫn giữ `legacy-booking-flow.md`.

![Tạo merge commit, push lên GitHub và kiểm tra working tree sạch](/images/day1/image51.png)
...
![PR số 13 không còn conflict sau khi push kết quả từ local](/images/day1/image52.png)

Chọn **Merge pull request** và xác nhận. PR chuyển sang **Merged**, hoàn thành quy trình xử lý bằng VS Code và Git local.

![PR số 13 đã merge thành công vào dev_1](/images/day1/image53.png)
