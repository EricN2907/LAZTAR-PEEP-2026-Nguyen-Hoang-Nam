...
![git init creates an empty repo at D:/LazTar/demo-git](/images/day1/image1.png)
...
![git remote before and after adding origin](/images/day1/image2.png)
...
![git remote add origin and git remote -v](/images/day1/image3.png)
![git remote -v showing fetch/push URLs](/images/day1/image4.png)
...
![git clone downloads the repo to local machine](/images/day1/image5.png)
...
- Run `git log --oneline -3` — local has 3 latest commits, `HEAD` and `origin/main` are both at commit `f5373ea (Fix name)`, fully in sync.

![git log --oneline -3: local is in sync with remote](/images/day1/image6.png)

- Create a new commit directly on GitHub named **"Add instructions for building production site"** (hash `2855f46`). The remote is now ahead of local.

![GitHub has a new commit "Add instructions for building production site"](/images/day1/image7.png)

- After running `git fetch origin` → Git downloads the info and updates `origin/main` to `2855f46`. Run `git log main..origin/main --oneline` → see the new remote commit. Run `git diff main origin/main` → see the README changes. But the local `main` branch still does not have that commit.

![git fetch origin: diff is visible but local is not yet merged](/images/day1/image8.png)

> **Conclusion:** `git fetch` only downloads tracking info — it does **not** automatically merge into the working branch.

![git log --oneline -3 after fetch: local still at f5373ea, missing 2855f46](/images/day1/image9.png)
...
![git pull origin main: Fast-forward, README.md updated](/images/day1/image10.png)
...
![git fetch + git merge origin/main: manual Fast-forward](/images/day1/image11.png)
...
![git status: modified file and untracked folder visible](/images/day1/image12.png)
...
![git status after pull: "nothing to commit, working tree clean"](/images/day1/image13.png)
![git status is clean after committing](/images/day1/image14.png)
...
![git branch: lists branches, * marks the current one](/images/day1/image15.png)
![git branch + git switch: list branches then switch to main](/images/day1/image16.png)
...
![git checkout: "Your branch is up to date with 'origin/main'"](/images/day1/image17.png)
...
![git checkout: "Your branch is up to date with 'origin/main'"](/images/day1/image18.png)
...
![git switch -c, git add, git commit, git switch main, git push](/images/day1/image19.png)
...
![git commit --amend: hash changes from 887d8aa to 1d481b8](/images/day1/image20.png)
...
![git switch -c feature-demo + git add + git commit "Add feature demo"](/images/day1/image21.png)
![git log + git reset --soft HEAD~1 + git log + git status](/images/day1/image22.png)
...
**Before rebase**: `feature-demo` branched off from `main` — two branches running in parallel.

![git switch -c feature-demo + commit "Add feature demo": history starts diverging](/images/day1/image23.png)
![Git graph diagram: main and feature-demo have diverged](/images/day1/image24.png)

**After rebase**: Run `git switch feature-demo` then `git rebase main` → `feature-demo` history is replayed on top of `main`, now linear and clean.
...
![git rebase main: "Current branch feature-demo is up to date"](/images/day1/image25.png)
...
- After `echo "# WIP note" >> README.md`, run `git status` → shows `modified: README.md`. This is the state **before stashing**.

![git status: README.md is modified before stash](/images/day1/image26.png)

- Run `git stash` → Git saves changes to stash (`Saved working directory... WIP on feature-demo`). Then `git stash pop` → changes are restored, `README.md` is `modified` again.

![git stash + git stash pop: save then restore changes](/images/day1/image27.png)
...
- Create branch `rebase-demoo` and make 3 commits (`fix A`, `fix B`, `fix C`) to `README.md`. Run `git log --oneline -4` → shows 3 separate commits.

![3 commits fix A, fix B, fix C created on rebase-demoo](/images/day1/image28.png)

- `git log --oneline -4` on branch `rebase-demooo` — `HEAD` is `squash fix A B C` (already squashed), below are the original commits `fix C`, `fix B`, `fix A`.

![git log after rebase -i: HEAD is "squash fix A B C"](/images/day1/image29.png)

- The Vim editor screen for `git rebase -i` — shows 3 `pick` lines for 3 commits. Change lines 2 and 3 from `pick` to `s` (squash) to merge them into the first commit.

![git rebase -i editor: pick/squash 3 commits](/images/day1/image30.png)
...
- Switch to `main` then run `git cherry-pick a9c80b8` → commit `squash fix A B C` is copied to `main` with a new hash `1d93759`. Run `git log --oneline -3` → the commit appears on `main`.
...
![git cherry-pick: "squash fix A B C" commit appears on main branch](/images/day1/image31.png)
...
The working tree is clean. At the time of this screenshot, local `dev_1` is 15 commits ahead of `origin/dev_1`. This counts unpublished commits, not the number of minutes in the reservation policy.

![Checking the working tree and local branches in BE](/images/day1/image32.png)

Push `dev_1` and both feature branches successfully, setting upstream tracking for the new branches.

![Pushing the target and feature branches to GitHub](/images/day1/image33.png)
...
![Opening the reservation policy on the feature branch and dev_1](/images/day1/image34.png)

GitHub reports **Can't automatically merge**. The diff below shows the feature branch's change from the common base, **5 → 10 minutes**. It does not directly compare the final values of **10 and 15 minutes**.

![Selecting base and compare; GitHub cannot merge automatically](/images/day1/image35.png)
...
![Entering the title for the reservation timeout pull request](/images/day1/image36.png)

PR **#12** lists `docs/booking-policy.md` as conflicting. **Resolve conflicts** is available because this is a simple content conflict.

![PR number 12 reports a conflict in booking-policy.md](/images/day1/image37.png)
...
![The web editor shows the competing 10-minute and 15-minute values](/images/day1/image38.png)
...
![Keeping 15 minutes and marking the file as resolved](/images/day1/image39.png)
...
![The PR has no conflicts after committing the resolution on GitHub](/images/day1/image40.png)
...
![PR number 12 successfully merged into dev_1](/images/day1/image41.png)
...
![Comparing the booking policy feature branch with dev_1](/images/day1/image42.png)
...
![Creating the booking policy update pull request](/images/day1/image43.png)

PR **#13** lists three conflicting files. Hovering over the disabled **Resolve conflicts** button shows that these conflicts cannot be resolved in the web editor.

![GitHub reports conflicts too complex for its web editor](/images/day1/image44.png)
...
![Checking the clean working tree and switching to the policy feature branch](/images/day1/image45.png)
...
![The terminal reports content and modify/delete conflicts after merging](/images/day1/image46.png)
...
![Cancellation conflict: Current is 2 hours, Incoming is 4 hours](/images/day1/image47.png)

![Payment conflict: Current is 30%, Incoming is 50%](/images/day1/image48.png)
...
The cancellation policy now says **4 hours**. Source Control no longer has a **Merge Changes** group, and the content changes are staged.

![Resolved content and the files listed under Staged Changes](/images/day1/image49.png)
...
![Reviewing the staged diff before completing the merge](/images/day1/image50.png)
...
Commit `bd6a9e1` is created and pushed successfully, and the working tree is clean. The commit command above matches the screenshot. Although its message says `retire legacy flow`, the actual commit keeps `legacy-booking-flow.md`.

![Creating the merge commit, pushing it, and confirming a clean working tree](/images/day1/image51.png)
...
![PR number 13 has no conflicts after pushing the local resolution](/images/day1/image52.png)

Select **Merge pull request** and confirm. The PR changes to **Merged**, completing the VS Code and local Git workflow.

![PR number 13 successfully merged into dev_1](/images/day1/image53.png)
